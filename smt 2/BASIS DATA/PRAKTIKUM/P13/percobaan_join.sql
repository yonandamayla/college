...
-- INNER JOIN
-- 1. Tampilkan nama kelas berikut nama prodinya
SELECT kelas.nama_kelas, prodi.nama_prodi
FROM kelas
INNER JOIN prodi ON kelas.kode_prodi = prodi.kode_prodi;

-- 2. Menggunakan Alias (AS)
SELECT k.nama_kelas, p.nama_prodi
FROM kelas AS k
INNER JOIN prodi AS p ON k.kode_prodi = p.kode_prodi;

-- 3. JOIN beberapa tabel
-- Tampilkan dosen mengajar di kelas apa pada hari apa
SELECT 
    j.kode_jadwal, 
    d.nama_dosen, 
    k.nama_kelas, 
...
INNER JOIN kelas k ON j.kode_kelas = k.kode_kelas
INNER JOIN hari n ON j.kode_hari = n.kode_hari

-- 4. INNER JOIN dengan WHERE
-- Menampilkan nama kelas dan nama prodi, 
-- yang nama kelasnya adalah kelas 'A'
SELECT k.nama_kelas, p.nama_prodi
FROM kelas k
INNER JOIN prodi p ON k.kode_prodi = p.kode_prodi
WHERE k.nama_kelas LIKE '%A%';

-- 1. LEFT OUTER JOIN
-- Menampilkan kelas yang belum ada prodinya
SELECT 
k.nama_kelas, 
p.nama_prodi
FROM kelas k
...
-- 2. OUTER JOIN dengan WHERE
-- Menampilkan kelas yang belum ada prodinya saja
SELECT 
    k.nama_kelas, 
    p.nama_prodi
FROM
...
-- 3. RIGHT OUTER JOIN
-- sama dengan LEFT OUTER JOIN, hanya beda tempat NULL-nya saja
-- LEFT OUTER JOIN --> yang penuh di kiri (LEFT)
-- RIGHT OUTER JOIN --> yang penuh di kanan (RIGHT)
SELECT 
    k.nama_kelas, 
    p.nama_prodi
FROM 
...
-- 4. FULL OUTER JOIN
-- Gabungan LEFT OUTER JOIN dan RIGHT OUTER JOIN
SELECT 
    k.nama_kelas, 
    p.nama_prodi
FROM kelas k
...
-- 1. SELECT JOIN Implisit
-- Tidak ada kata - kata JOIN pada SQL-nya
SELECT 
    k.nama_kelas, 
    p.nama_prodi
...
-- 2. INNER JOIN implisit dengan alias (AS)
SELECT 
    k.nama_kelas, 
    p.nama_prodi
FROM kelas k, prodi p
...
-- TUGAS 
-- no 1 tammpilkan dosen pengampu dan nama mata kuliah
SELECT DISTINCT
    d.nama_dosen, 
    m.nama_mk
FROM dosen d
INNER JOIN jadwal j ON d.kode_dosen = j.kode_dosen
INNER JOIN mk m ON j.kode_mk = m.kode_mk
ORDER BY d.nama_dosen ASC;

-- no 2 tampilkan kode ruang, nama ruang, hari dipakai, dari jam, sampai jam Pada hari selasa
SELECT 
    r.kode_ruang, 
    r.nama_ruang, 
    h.nama_hari, 
...
-- no 3 tampilkan dosen yang tidak mendapatkan jadwal menagajar
SELECT 
    d.nama_dosen, 
    j.kode_jadwal
FROM dosen d
LEFT OUTER JOIN jadwal j ON d.kode_dosen = j.kode_dosen
WHERE j.kode_jadwal IS NULL;

-- no 4 tampilkan field mata kuliah dan pengampu mata kuliah. 
-- Tampilkan semua mata kuliah meskipun tidak ada pengampu nya
SELECT 
    m.nama_mk, 
    d.nama_dosen
FROM mk m
LEFT OUTER JOIN jadwal j ON m.kode_mk = j.kode_mk
LEFT OUTER JOIN dosen d ON j.kode_dosen = d.kode_dosen;


-- no 5 tampilkan nama-nama dosen beserta jumlah jadwal mereka, 
-- jika dosen tidak mendapatkan jadwal, maka jumlah_jadwal_mengajarnya = 0
SELECT 
    d.nama_dosen, 
